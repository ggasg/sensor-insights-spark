...
class InsightTests extends AnyFunSuite with SharedSparkContext with Matchers {
  ...
  test("Should detect anomalies per sensor independently") {
    val testData = Seq(
      (Timestamp.valueOf("2026-02-17 14:00:00"), "sensor1", Array(20.0, 68.0), 100.0, 1013.0),
      (Timestamp.valueOf("2026-02-17 14:01:00"), "sensor1", Array(25.0, 77.0), 100.0, 1013.0),  // sensor1: +5°C jump
      (Timestamp.valueOf("2026-02-17 14:00:00"), "sensor2", Array(15.0, 59.0), 100.0, 1013.0),
      (Timestamp.valueOf("2026-02-17 14:01:00"), "sensor2", Array(15.5, 59.9), 100.0, 1013.0)   // sensor2: normal
    ).toDF("event_timestamp", "chip_id", "temperature", "altitude", "pressure")
